...
public class graphs2 {
    ...
    public static void bfsUtil(ArrayList<Edge> graph[],int v,boolean[] visited){ //O(V+E)
        for(int i=0;i<v;i++){
            visited[i]=false;
        }
    ...
    public static void dfs(ArrayList<Edge> graph[],int v){
        ...
        for(int i=0;i<graph.length;i++){
            if(!visited[i]){
                dfsUtil(graph,i,v,visited);
            }
        }
    }

    public static void dfsUtil(ArrayList<Edge> graph[],int curr,int v,boolean visited[]){ //O(V+E)
        if(visited[curr]){
            return;
        }
        ...
        for(int i=0;i<graph[curr].size();i++){
            dfsUtil(graph,graph[curr].get(i).destination,v,visited);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        dfs(graph1,5);
        System.out.println();
        dfsUtil(graph1, 0, v, new boolean[7]);
